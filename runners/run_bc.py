...
def betweenness_centrality(X,edge_index,labels,test_y,test_idx,genes,model):
    ...
    auc_total = metrics.roc_auc_score(test_y,BC_y[test_idx])

    precision, recall, thresholds= metrics.precision_recall_curve(test_y,BC_y[test_idx])
    ...
    average_precision = metrics.average_precision_score(test_y,BC_y[test_idx])
    # print(f'\nTest average_precision: {average_precision*100:.2f}%')

    model.iloc[0:count_essential_genes, :]=1
    ess=model.iloc[0:count_essential_genes, :]

    # print(ess)
    # print(ess.shape)
    
    model.iloc[count_essential_genes:model.shape[0], :]=0
    ness=model.iloc[count_essential_genes:model.shape[0], :]

    model = model.reindex(genes_index.index) 
    
    y=genes_index.iloc[:,0]
    BC_y=model.iloc[:,0]

    mcc=metrics.matthews_corrcoef(BC_y[test_idx].astype(int),test_y)
    # print(f'\nTest matthews corrcoef: {mcc*100:.2f}%')

    test_acc = accuracy_score(BC_y[test_idx].astype(int),test_y)
    # print(f'\n acc: {test_acc*100:.2f}%')


    return auc_total,auc_precision_recall,average_precision,mcc,test_acc

def main(args,model, name='', seed=200, save=True):
    ...
    auc_total,auc_precision_recall,average_precision,mcc,test_acc=betweenness_centrality(X,edge_index,labels,test_y,test_idx,genes,model)
  # ---------------------------------------------------
...
if __name__ == '__main__':
    ...
    (edge_index, edge_weights), X,labels, _, \
    _, _, genes = tools.get_data(
        args.__dict__, seed=1, weights=True)
    ...
    model = results_df.reindex(genes_index.index) 
    ...
    if args.n_runs:
        ...
        for i in range(args.n_runs):
            ...
            auc_total,auc_precision_recall,precision,mcc,test_acc= main(args,model, name=name, seed=i)
