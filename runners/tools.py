...
def get_data(args, seed=0, parse=True, weights=False):
    ...
    (edges, edge_weights), X,labels, train_ds, test_ds, genes = utils.data(
    ...
    # print(f'Number of features before feature selectio: {X[red_idx].shape[1]}')
    ...
    train_y = torch.tensor(train['Label'].astype(int).values, dtype=torch.float32)
    val_y = torch.tensor(val['Label'].astype(int).values, dtype=torch.float32)
    test_y = torch.tensor(test_ds['Label'].astype(int).values, dtype=torch.float32)
    ...
    if not parse:
        return (edge_index, edge_weights), X,labels, train_ds, test_ds, genes

    return (edge_index, edge_weights), X,labels, (train_idx, train_y), (val_idx, val_y), (test_idx, test_y), genes