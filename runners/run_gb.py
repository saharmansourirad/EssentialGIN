...
def gradient_boosting_classifier(train_idx, train_y, X, test_y, test_idx):
    ...
    GB_y = model.predict_proba(X[test_idx])[:, 1]

    # Evaluation Metrics
    auc_total = roc_auc_score(test_y, GB_y)
    precision, recall, _ = precision_recall_curve(test_y, GB_y)
    auc_precision_recall = auc(recall, precision)
    average_precision = average_precision_score(test_y, GB_y)

    # Threshold predictions at 0.5 for binary classification
    mcc = matthews_corrcoef(test_y, (GB_y >= 0.5).astype(int))
    test_acc = accuracy_score(test_y, (GB_y >= 0.5).astype(int))
...
def main(args, name='', seed=200, save=True):
    ...
    auc_total,auc_precision_recall,average_precision,mcc,test_acc=gradient_boosting_classifier(train_idx, train_y, X, test_y, test_idx)
  # ---------------------------------------------------
