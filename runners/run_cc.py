...
def clustering_coefficient(X,edge_index,labels,test_y,test_idx,genes):
    ...
    auc_total = metrics.roc_auc_score(test_y,CC_y[test_idx])

    precision, recall, thresholds= metrics.precision_recall_curve(test_y,CC_y[test_idx])
    ...
    average_precision = metrics.average_precision_score(test_y,CC_y[test_idx])
    ...
    y=genes_index.iloc[:,0]
    CC_y=results_df.iloc[:,0]

    mcc=metrics.matthews_corrcoef(CC_y[test_idx].astype(int),test_y)
    # print(f'\nTest matthews corrcoef: {mcc*100:.2f}%')

    test_acc = accuracy_score(CC_y[test_idx].astype(int),test_y)
    # print(f'\n acc: {test_acc*100:.2f}%')
...
def main(args, name='', seed=200, save=True):
    ...
    auc_total,auc_precision_recall,average_precision,mcc,test_acc=clustering_coefficient(X,edge_index,labels,test_y,test_idx,genes)
  # ---------------------------------------------------
